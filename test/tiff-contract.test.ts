...
vi.mock('utif2', () => ({
  default: {
    ...
    decodeImage() {},
    toRGBA8(ifd: { width: number; height: number }) {
      return new Uint8Array(ifd.width * ifd.height * 4).fill(255);
    }
  }
}));

let decodeTiff: typeof import('../src/decoders/tiff.js').decodeTiff;
let getTiffPageCount: typeof import('../src/decoders/tiff.js').getTiffPageCount;

beforeAll(async () => {
  ({ decodeTiff, getTiffPageCount } = await import('../src/decoders/tiff.js'));
});
