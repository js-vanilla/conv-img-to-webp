...
describe('checkWebPFeature', () => {
  it('returns false in runtimes without Image', async () => {
    expect(await checkWebPFeature('lossy')).toBe(false);
  });

  it('keeps the callback-compatible exported alias', async () => {
    await new Promise<void>(resolve => {
      check_webp_feature('lossless', (feature, supported) => {
        expect(feature).toBe('lossless');
