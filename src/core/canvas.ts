...
export function createCanvas(
  width: number,
  height: number,
  factory?: ConvertToWebPOptions<WebPOutputKind>['canvasFactory'],
  options: ConvertToWebPOptions<WebPOutputKind> = {}
): CanvasLike {
...
export function get2dContext(
  canvas: CanvasLike,
  settings: CanvasRenderingContext2DSettings = { willReadFrequently: true }
): Canvas2DContext {
  const ctx = canvas.getContext('2d', settings) as Canvas2DContext | null;
  if (!ctx) throw new EncodeError('Unable to create a 2D canvas context.');
  return ctx;
}

export function closeBitmap(bitmap: ImageBitmap | HTMLImageElement | undefined): void {
  if (bitmap && 'close' in bitmap && typeof bitmap.close === 'function') bitmap.close();
}

export function rasterToCanvas(raster: RasterImage, options: ConvertToWebPOptions<WebPOutputKind> = {}): CanvasLike {
