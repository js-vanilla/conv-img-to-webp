...
const JP2_SIGNATURE = [0x6a, 0x50, 0x20, 0x20, 0x0d, 0x0a, 0x87, 0x0a] as const;
const PNG_SIGNATURE = [0x50, 0x4e, 0x47] as const;
...
const WEBP_FEATURE_TEST_IMAGES: Record<WebPFeature, string> = Object.freeze({
...
function toBytes(bytesOrBuffer: Uint8Array | ArrayBuffer | ArrayBufferView | null | undefined): Uint8Array {
  if (bytesOrBuffer instanceof Uint8Array) return bytesOrBuffer;
  if (bytesOrBuffer instanceof ArrayBuffer) return new Uint8Array(bytesOrBuffer);
...
function isKnownImageType(type: string | undefined): type is SupportedImageType {
  return type === JPEG || type === PNG || type === GIF || type === TIFF || type === JP2 || type === J2K;
}

function runWebPFeatureCheck(feature: WebPFeature): Promise<boolean> {
  const testImage = WEBP_FEATURE_TEST_IMAGES[feature];
  if (!testImage) return Promise.reject(new TypeError(`Unsupported WebP feature: ${String(feature)}.`));
...
export function detectImageType(
  bytesOrBuffer: Uint8Array | ArrayBuffer | ArrayBufferView | null | undefined,
  mimeHint?: string
): SupportedImageType | undefined {
  const hinted = normalizeMimeType(mimeHint);
  const bytes = toBytes(bytesOrBuffer);

  if (bytes.length >= 12 && bytes[0] === 0x00 && bytes[1] === 0x00 && bytes[2] === 0x00 && bytes[3] === 0x0c && matches(bytes, 4, JP2_SIGNATURE)) return JP2;
  if (bytes.length >= 4 && bytes[0] === 0xff && bytes[1] === 0x4f && bytes[2] === 0xff && bytes[3] === 0x51) return J2K;
  if (bytes.length >= 3 && bytes[0] === 0xff && bytes[1] === 0xd8 && bytes[2] === 0xff) return JPEG;
  if (bytes.length >= 8 && bytes[0] === 0x89 && matches(bytes, 1, PNG_SIGNATURE) && bytes[4] === 0x0d && bytes[5] === 0x0a && bytes[6] === 0x1a && bytes[7] === 0x0a) return PNG;
  if (matches(bytes, 0, GIF87A_SIGNATURE) || matches(bytes, 0, GIF89A_SIGNATURE)) return GIF;
...
export function isTiff(type?: string): boolean {
  return normalizeMimeType(type) === TIFF;
}

export function isJpeg2000(type?: string): boolean {
  const normalized = normalizeMimeType(type);
  return normalized === JP2 || normalized === J2K;
}

export function checkWebPFeature(feature: WebPFeature): Promise<boolean>;
export function checkWebPFeature(feature: WebPFeature, callback: WebPFeatureCallback): void;
export function checkWebPFeature(feature: WebPFeature, callback?: WebPFeatureCallback): Promise<boolean> | void {
  const result = runWebPFeatureCheck(feature);
  if (!callback) return result;

  result.then(
    supported => callback(feature, supported),
    () => callback(feature, false)
  );
}

export const check_webp_feature: typeof checkWebPFeature = checkWebPFeature;
